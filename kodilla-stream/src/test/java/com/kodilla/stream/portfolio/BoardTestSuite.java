...
class BoardTestSuite {
    ...
    public void testAddTaskListAverageWorkingOnTask() {
        ...
        //summing days
        int summingDays = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(t -> t.getCreated())
    ...
    public void testAddTaskListAverageWorkingOnTask2() {
        ...
        OptionalDouble average = project.getTaskLists().stream()
                ...
                .flatMap(tl -> tl.getTasks().stream())
                .map(t -> t.getCreated())
                .mapToInt(n -> (int) ChronoUnit.DAYS.between(n, LocalDate.now()))
                .average();

        //Then
        assertEquals(10, average.getAsDouble(), 0.1);
